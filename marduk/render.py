...
``render_value(val)`` returns a ``(text/plain, text/html)`` pair the kernel
emits as a Jupyter MIME bundle. Both forms are depth-bounded so a
pathological structure (a malformed App tree, a Pin loop) can't blow the
stack while we're trying to *show* it.
...
- Law ``{a n b}``     → ``"{'name' arity body}"``; ``name`` is decoded via
                        ``nat_str`` and shown single-quoted (or decimal if
                        the nat doesn't decode to printable UTF-8).
- App ``f x``         → ``"(f x)"``.

The HTML output mirrors ``gallowglass/bootstrap/value_render.HtmlFormatter``:
inline ``style="…"`` (Jupyter strips ``<style>`` blocks) and a cross-theme
palette that reads on both light and dark backgrounds.
"""
...
from .runtime.plan import is_app, is_law, is_nat, is_pin, nat_str
...
# Cross-theme-friendly palette (matches gallowglass's HtmlFormatter).
...
def _text(val, *, depth: int, pretty: bool, max_depth: int) -> str:
    ...
    if is_nat(val):
        return str(val)
    if is_pin(val):
        if pretty and is_law(val.val):
            return f"<{{{_law_name_text(val.val)}…}}>"
        inner = _text(val.val, depth=depth + 1, pretty=pretty, max_depth=max_depth)
        return f"<{inner}>"
    if is_law(val):
        body = _text(val.body, depth=depth + 1, pretty=pretty, max_depth=max_depth)
        return f"{{{_law_name_text(val)} {val.arity} {body}}}"
    if is_app(val):
        f = _text(val.fun, depth=depth + 1, pretty=pretty, max_depth=max_depth)
        a = _text(val.arg, depth=depth + 1, pretty=pretty, max_depth=max_depth)
        return f"({f} {a})"
    return repr(val)
...
def _html(val, *, depth: int, pretty: bool, max_depth: int) -> str:
    ...
    if is_nat(val):
        return f'<span style="{_NAT}">{val}</span>'
    if is_pin(val):
        if pretty and is_law(val.val):
            return (
                f'<span style="{_MUTED}">&lt;{{</span>'
                f"{_law_name_html(val.val)}"
                f'<span style="{_MUTED}">…}}&gt;</span>'
            )
        inner = _html(val.val, depth=depth + 1, pretty=pretty, max_depth=max_depth)
        return (
            f'<span style="{_MUTED}">&lt;</span>'
            f"{inner}"
            f'<span style="{_MUTED}">&gt;</span>'
    ...
    if is_law(val):
        ...
        return (
            ...
            f'<span style="{_NAT}">{val.arity}</span> '
            f"{body}"
            f'<span style="{_MUTED}">}}</span>'
        )
    if is_app(val):
        f = _html(val.fun, depth=depth + 1, pretty=pretty, max_depth=max_depth)
        a = _html(val.arg, depth=depth + 1, pretty=pretty, max_depth=max_depth)
        return (
            f'<span style="{_MUTED}">(</span>'
            f"{f} {a}"
            f'<span style="{_MUTED}">)</span>'
...
def _law_name_html(law) -> str:
    if is_nat(law.name):
        ...
        return f'<span style="{_NAT}">{law.name}</span>'
