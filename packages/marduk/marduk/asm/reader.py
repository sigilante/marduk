...
``parse`` / ``pseq`` / ``parseMany`` block). Output is a PLAN ``Val``
using Marduk's runtime constructors — not a separate surface AST. This
matches PlanAssembler's semantics: parse and macroexpand both operate
on PLAN values directly.

Encoding rules mirror PlanAssembler's ``natE`` / ``strE`` / ``symE`` /
``listE`` / ``curlE`` / ``brakE``:

* ``(...)``    → ``array(xs)`` — a left-associated App chain headed by ``Nat(0)``
* ``[...]``    → ``array([Nat("#brak")] + xs)``
* ``{...}``    → ``array([Nat("#curl")] + xs)``
* ``"foo"``    → ``natE(strNat("foo"))`` = ``App(Nat(1), Nat(strNat("foo")))``
* bare nat lit → ``natE(n)`` = ``App(Nat(1), Nat(n))``
* bare symbol  → ``Nat(strNat(sym))``
* ``sym(...)`` / ``sym"..."`` (no whitespace between) →
  ``array([Nat("#juxt"), v, body])``

``;`` starts a line comment that runs to the next ``\\n``. Tabs and CRs
are intentionally treated as symbol characters, matching upstream.

The lifted-from-plan-kernel form: this file is a near-verbatim port of
``packages/plan-kernel/plan_kernel/parser.py``; the only changes are
swapping the runtime constructor imports.
"""

from __future__ import annotations

from ..runtime import App, Nat
...
class ParseError(Exception):
    ...
    Carries 1-indexed ``line`` and ``col`` of the offending position.
    """
...
def _array(xs):
    """``adt 0 xs``. ``[]`` → ``Nat(0)``; non-empty → left-associated App chain."""
    if not xs:
        return Nat(0)
    val = Nat(0)
    for x in xs:
...
def _brak_e(xs):
    return _array([Nat(str_nat("#brak"))] + xs)


def _curl_e(xs):
    return _array([Nat(str_nat("#curl"))] + xs)


def _juxt_e(v, body):
    return _array([Nat(str_nat("#juxt")), v, body])


def _nat_e(n: int):
    return App(Nat(1), Nat(n))
...
def parse(text: str):
    """Parse a single Plan Asm form from ``text``. Trailing content after
    the form is ignored — use :func:`parse_many` for a sequence."""
    cur = _Cursor(text)
    return _parse(cur)


def parse_many(text: str):
    """Parse a sequence of top-level Plan Asm forms. Returns ``list[Val]``."""
    cur = _Cursor(text)
    items = []
    _eat(cur)
    while not cur.at_eof():
