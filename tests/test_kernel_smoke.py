...
The :class:`MardukKernel` class itself is defined inside :func:`_kernel_main`
...
def test_module_imports_without_launching_kernel():
    """Importing ``marduk.kernel`` must not pull in ``ipykernel`` (which
    is lazy-imported inside ``_kernel_main``)."""
    import marduk.kernel as kernel
...
def test_main_module_imports():
    import marduk.__main__   # noqa: F401


def test_install_kernelspec_writes_kernel_json(tmp_path):
    pytest.importorskip("jupyter_client")
    from marduk.kernel import _install_kernelspec
    ...
    # Identity
    assert spec["display_name"] == "Marduk (PLAN)"
    assert spec["language"] == "plan"

    # argv launches the kernel via `python -m marduk` with the connection file.
    argv = spec["argv"]
    assert argv[0] == sys.executable
    assert "-m" in argv and "marduk" in argv
...
def test_install_kernelspec_install_path_contains_marduk(tmp_path):
    pytest.importorskip("jupyter_client")
    from marduk.kernel import _install_kernelspec

    installed = _install_kernelspec(user=False, prefix=str(tmp_path))
    # KernelSpecManager normalizes to lowercase kernel name.
    assert installed.endswith(os.sep + "marduk")


def test_cli_install_subcommand(tmp_path, capsys):
    pytest.importorskip("jupyter_client")
    from marduk.kernel import _cli_main

    rc = _cli_main(["__main__", "install", "--prefix", str(tmp_path)])
    assert rc == 0

    captured = capsys.readouterr()
    assert "Installed Marduk kernelspec at" in captured.out
...
def test_cli_install_creates_loadable_kernel_json(tmp_path):
    ...
    # Spec lives at <prefix>/share/jupyter/kernels/marduk/kernel.json
    spec_path = tmp_path / "share" / "jupyter" / "kernels" / "marduk" / "kernel.json"
...
def test_module_can_be_invoked_via_python_dash_m(tmp_path):
    """``python -m marduk install --prefix tmp`` must succeed end-to-end."""
    pytest.importorskip("jupyter_client")
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    result = subprocess.run(
        [sys.executable, "-m", "marduk", "install", "--prefix", str(tmp_path)],
        cwd=repo_root,
        capture_output=True,
        text=True,
        timeout=30,
    ...
    assert "Installed Marduk kernelspec at" in result.stdout


def test_format_error_with_loc():
    from marduk.kernel import _format_error
    envelope = {
        "stage": "parse",
